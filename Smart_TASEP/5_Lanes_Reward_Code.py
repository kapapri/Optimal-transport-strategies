...
# surroundings reward (after movement)    
    ...
    a = 0.1
    # gradient_factor = a * ((1 + a) / a) ** speed - a
    gradient_factor = speed ** 4

    desired_distance = int(L / 2) * (1 - gradient_factor)
    distance = abs(int(L / 2) - nextX)
    # print('desired_distance', desired_distance)
    # print('distance from the center', distance)

    reward += -2*int(abs(desired_distance - distance)/int(L/2))
