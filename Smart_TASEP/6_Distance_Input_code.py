...
def get_state_patch(system, L, Ycenter):
 ...
 # when the system is the same size than the training patch (L x L)
    ...
    old_input = np.concatenate((fast_channel, slow_channel)).flatten()

    distance_center = (Ycenter - int(newLy / 2))/int(newLy / 2)
    state = np.append(old_input, distance_center)
...
def do_training(num_episodes, L, density, Nt, newLx, newLy, boundary_lane, log = False):
    ...
    for i_episode in tqdm(range(num_episodes)):
        ...
        before_right_fast, after_right_fast = 0, 0
        before_right_slow, after_right_slow = 0, 0          
        for t in range(Nt):
            for i in range(newLx*newLy):
                fast_particles, fast_particles_up  = 0, 0
                slow_particles, slow_particles_down = 0, 0
                before_fast, after_fast = 0, 0
                before_slow, after_slow = 0, 0
                reward_fast, reward_slow = 0, 0
                # Random sampling in the new lattice to apply the training
                Xcenter = random.randint(0, newLx-1)
                Ycenter = random.randint(0, newLy-1)
               # checking all works without random sampling (for a 6x6 patch at a 6x6 system)
                # Xcenter = 3
                # Ycenter = 3

                # counting particles patch before
                for i in range(L):
                    for j in range(L):
                        X_sys, Y_sys = get_coordinates_from_patch(i, j, Xcenter, Ycenter, L, newLx, newLy)
                        if lattice[X_sys][Y_sys] == 1 and Y_sys < boundary_lane:
                            before_fast += 1
                            # choice
                        elif lattice[X_sys][Y_sys] == 0.8 and Y_sys >= boundary_lane:
                            before_slow += 1
                ...
                if log == True:
                    ...
                    print("\n Xcenter(row), Ycenter(column) =", (Xcenter, Ycenter))
                    print("\n state before (fast) \n", state[:int(L*L)].reshape(L,L))
                    print("state before (slow) \n", state[int(L*L):(2*L*L)].reshape(L,L))
                    print("state before (distance to the center): ", state[2*L*L])
                    print("\nright fast particles before", before_fast)
                    print("right slow particles before", before_slow)
                ...
                if log == True:
                    print("\n - lattice_site chosen (= x*L + y):", lattice_site)
                ...
                if lattice[selectedX][selectedY] != 0:
                    ...
                    if log == True:
                        print("state after (fast) \n", next_state[0][:int(L*L)].numpy().reshape(L,L))
                        print("state after (slow) \n", next_state[0][int(L*L):(2*L*L)].numpy().reshape(L,L))
                        print("state after (distance to the center):", next_state[0][2*L*L].numpy())
                ...
                for i in range(L):
                    for j in range(L):
                        ...
                        #choice
                            ...
                            after_slow += 1
                reward_fast = int(after_fast - before_fast)
                reward_slow = int(after_slow - before_slow)
                # reward += 10*(reward_fast + reward_slow)

                if log == True:
                    print("\n lattice after \n", lattice)
                    print("\nright fast particles after", after_fast)
                    print("right slow particles after", after_slow)
                    print('fast reward', reward_fast)
                    print('slow reward', reward_slow)

                # counting particles system
...
if __name__ == '__main__': 
    ...
    L = 6                   # squared patches for the training
    ...
    if Post_training:
            ...
            for t in tqdm(range(Nt)):
                ...
                for i in range(newLx*newLy):
                   ...
                   # patch with the same size than the system and sampling always from the center
                    ...
                    # state = get_state_patch(lattice, L, Ycenter)
                    state = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
        ...
        ani.save("./Movie"+ str(newLx) + "x" + str(newLy)  + "_runs" + str(runs) + ".gif", fps = 8)
        ...
        with open(filename, 'wb') as file:
            movie_storage = np.zeros(((newLx*newLy)*Nt + 1, newLx, newLy))  
            movie_storage = np.zeros(((newLx*newLy)*Nt//movie_divisions + 1, newLx, newLy)) 
