...
# move
def step(lattice, X, Y, L, part, log = False):
    newX = -1
    newY = -1
    # periodic boundaries
        ...
        for t in range(Nt):
            for i in range(L*L):                
                ...
                if lattice[selectedX][selectedY] != 0:
                    ...
                    reward, next_state, current_along = step(lattice, selectedX, selectedY, L, part, log) # update particle's position and do stochastic part
...
if __name__ == '__main__': 
    ...
    L = 5                   # square patches for the training                   
    ...
    if Jessie_we_need_to_train_NN:
        ...
        do_training(num_episodes, L, density, Nt, part, log) 
