...
# move
def step(lattice, X, Y, Lx, Ly, part, log = False):
    newX = -1
    newY = -1
...
# surroundings reward (before movement)
    ...
    # print('newnextX with value', newnextX, lattice[newnextX][newY])  
    # print('newprevX with value', newprevX, lattice[newprevX][newY])  
    # print('newnextY with value', newnextY, lattice[newX][newnextY])  
    # print('newprevY with value', newprevY, lattice[newX][newprevY])  
...
def do_training(num_episodes, Lx, Ly, density, Nt, part, log = False):
    ...
    for i_episode in tqdm(range(num_episodes)):
        ...
        for t in range(Nt):
            for i in range(Lx*Ly):                
                ...
                if lattice[selectedX][selectedY] != 0:
                    ...
                    reward, next_state, current_along = step(lattice, selectedX, selectedY, Lx, Ly, part, log) # update particle's position and do stochastic part
        ...
        if part == 2:
            print("Training episode ", i_episode, " is over. Current = ", total_current, "; Selected empty sites / Lx*Ly = ", selected_empty_site / (Nt*Lx*Ly))
        rewards.append(score.numpy()) 
        current.append(total_current)
        empty_sites.append(selected_empty_site/ (Nt*Lx*Ly))
    ...
    plt.savefig(f"./Training_Reward_{part}_{Lx}x{Ly}.png", format="png", dpi=600) # only withOUT interactive python
    plot_empty_sites()
    plt.savefig(f"./Empty_Sites_Chosen{Lx}x{Ly}.png", format="png", dpi=600) # only withOUT interactive python
# plots
...
def plot_empty_sites():
    plt.figure(3)
    plt.xlabel('Episode duration')
    plt.ylabel('Number of empty sites chosen over Lx x Ly')
...
if __name__ == '__main__': 
    ...
    Jessie_we_need_to_train_more_NN = False
    Post_training =  True
    log = False
    log_post = False
    ...
    if Jessie_we_need_to_train_NN:
        ...
        do_training(num_episodes, Lx, Ly, density, Nt, part, log) 
    ...
    if Post_training:
        ...
        plot_current()
        plt.savefig(f"./Post_Current{part}_{runs}.png", format="png", dpi=600) # only withOUT interactive python
