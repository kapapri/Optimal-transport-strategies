...
# surroundings reward (after movement)    
    fast_count = 0
    slow_count = 0
    dtop,dbottom,dforward,dback = -1, -1, -1, -1
    d_c = 1 # Equilibrium distance
    K = 1 # LJ constant
    r = 1/d_c # Nearest neighbours

    if lattice[newX][prevY] != 0: #top particle once it moved

        dtop = abs(lattice[newX][prevY] - velocity)
        #LJtop = 1/r**2 - 1/r

        if lattice[newX][prevY] == 1: 
            fast_count += 1
        else:
            slow_count += 1 

    if lattice[newX][nextY] != 0: #bottom particle once it moved

        dbottom = abs(lattice[newX][nextY] - velocity)
        #LJbottom = 1/r**2 - 1/r

        if lattice[newX][nextY] == 1: 
            fast_count += 1
        else:
            slow_count += 1  

    if lattice[nextX][newY] != 0: #forward particle once it moved

        dforward = abs(lattice[nextX][newY] - velocity)
        #LJforward = 1/r**2 - 1/r

        if lattice[nextX][newY] == 1: 
            fast_count += 1
        else:
            slow_count += 1 

    if lattice[prevX][newY] != 0: #back particle once it moved

        dback = abs(lattice[prevX][newY] - velocity)
        #LJback = 1/r**2 - 1/r

        if lattice[prevX][newY] == 1: 
            fast_count += 1
        else:
            slow_count += 1 

    if lattice[newX][newY] == 1: # fast particle
        counting_reward = fast_count - slow_count
    elif lattice[newX][newY] != 0: # slow particle
        counting_reward = -fast_count + slow_count
    reward += counting_reward

    diff_velocities = int((-1)*(dtop + dbottom + dforward + dback))
    # reward += diff_velocities

    #LJ_function = (-K)*int(LJtop + LJbottom + LJforward + LJback)
    #reward += LJ_function
...
if __name__ == '__main__': 
    ...
    if Post_training:
        ...
        one_movie = np.zeros((((newLx*newLy)*Nt)//1000+1, newLx, newLy))  # All runs: All frames consecutively
            ...
            for t in tqdm(range(Nt)):
                ...
                for i in range(newLx*newLy):
                   ...
                   # patch with the same size than the system and sampling always from the center
                    ...
                    # one_movie[k] = lattice
                    # if k % 1000 == 0:  # Check if k is a multiple of 5
                    #     one_movie[k//1000] = lattice  # Store lattice in one_movie at every 5th index                    
                    # k += 1
        ...
        # ani.save("./Movie"+ str(newLx) + "x" + str(newLy)  + "_runs" + str(runs) + ".mp4", fps = 20)
